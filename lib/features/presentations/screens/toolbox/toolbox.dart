...
//     });
//
//     var url = Uri.parse('http://44.214.230.69:8000/toolbox_api/');
...
class _ToolBoxScreenState extends State<ToolBoxScreen> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final filteredDocs = responseData.where((doc) {
          ...
          print('Document Date: $docDate, Selected Date: $selectedDateLocal');
          return docDate.year == selectedDateLocal.year &&
              docDate.month == selectedDateLocal.month &&
              docDate.day == selectedDateLocal.day;
        }).toList();
        ...
        if (filteredDocs.isEmpty) {
          _showDialog('Documents Not Available', 'No documents available for the selected date.');
        }
      } else {
  ...
  Future<void> _uploadFile() async {
    if (_selectedFile == null) {
      _showDialog('Upload Rejected', 'Please select a file to upload.');
      return;
    }
    ...
    try {
      ...
      if (response.statusCode == 201) {
        _showDialog('Upload Successful', 'The document has been uploaded successfully.');
        setState(() {
          _selectedFile = null;
          _callToolBoxAPI();
        });
      } else {
        _showDialog('Upload Rejected', 'Failed to upload file. Please check the format and try again.');
      }
    } catch (e) {
      print('Error during file upload: $e');
      _showDialog('Upload Rejected', 'An error occurred during the upload. Please try again.');
    }
  ...
  Widget _buildProjectDropdown() {
    return DropdownButton<String>(
      value: selectedProject,
      icon: Icon(Iconsax.arrow_down, color: TColors.primaryColorButton),
      ...
      items: <String>['PROJECT 1', 'PROJECT 2', 'PROJECT 3']
        ...
        return DropdownMenuItem<String>(
          ...
          child: Row(
            children: [
              Icon(Icons.folder, color: TColors.primaryColorButton),
              SizedBox(width: 10),
